$(() => {
    ...
    $("#advertising-materials").change(() => {
        物料列表.forEach((当前元素, 当前元素下标) => {
            if (当前元素.name === $("#advertising-materials").val()) {
                ...
                $("#current-event-link span").html("<a href=\"./short-link-poster.html?amid="+物料列表[当前选中物料下标].id+"\" target=\"_blank\" rel=\"noopener noreferrer\">"+物料列表[当前选中物料下标].name+"</a>")
    ...
    $("button").click(() => {
        ...
        $.ajax({
            type: "GET",
            url: ChaosEnvironmentVariables.Apis.TKE+"/events/advertising-materials/poster/mini-program-short-link?amid="+物料列表[当前选中物料下标].id+"&suffix="+花名册[当前选中宣传人下标].Suffix,
