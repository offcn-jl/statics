(function () {
    ...
    // 接口路径
    ChaosApiPath = "https://api.chaos.jilinoffcn.com/test/events/login-module";  // 默认 ( 测试环境接口 )
    if (ChaosPath.split(".")[0].split("https://")[1] === "test") {
        ChaosApiPath = "https://api.chaos.jilinoffcn.com/test/events/login-module";  // 测试环境接口
    } else if (ChaosPath.split(".")[0].split("https://")[1] === "statics") {
        ChaosApiPath = "https://api.chaos.jilinoffcn.com/release/events/login-module";  // 生产环境接口
    } else {
        ChaosApiPath = "https://api.chaos.jilinoffcn.com/test/events/login-module";  // 本地环境 ( 测试环境接口 )
        // ChaosApiPath = "http://localhost:8080/test/events/login-module";  // 本地环境 ( 本地环境接口 )
    }
    ...
    let initTimer = setInterval(function () {
        ...
        if (typeof (document.getElementsByTagName("body")[0]) === "object") {
            ...
            // 加载地市定位代码表
            ChaosFunctions.DynamicLoading.JS(ChaosApiPath + "/postfix");
            ...
            initTimer = setInterval(function () {
                if (typeof ($_GET) === "object" && typeof (ChaosPlatformFind) === "function") {
                    ...
                    // 设置地市定位的资源的显示状态
                    ChaosFunctions.ShowByClass(ChaosLoacation.Key);

                    // 更新资源显示状态成功
                    ChaosFunctions.Logger({ Type: 'info', Info: '更新资源显示状态成功，当前显示的定位资源 Class 为 ' + ChaosLoacation.Key + ' !' });

                    // 填充 '19 课堂' 定位后缀
                    if (typeof ChaosLoacation.Key419 === 'string') {
                        ...
                        Object.keys(document.getElementsByClassName("chaos-link-19")).forEach(function (key) {
                            if (typeof document.getElementsByClassName("chaos-link-19")[key].getAttribute('chaos-id') === 'string' && document.getElementsByClassName("chaos-link-19")[key].getAttribute('chaos-id').length > 0) { // 判断是否填写了房间号 ( 先判断 chaos-id 属性是否为字符串，即是否设置了该属性，可以避免判断长度时报错 )
                                let roomID = document.getElementsByClassName("chaos-link-19")[key].getAttribute('chaos-id');
                                if (ChaosLoacation.Key419.substr(-2) === 'sc') {  // 判断当前定位是否是市场后缀
                                    if (typeof document.getElementsByClassName("chaos-link-19")[key].getAttribute('chaos-id-market') === 'string' && document.getElementsByClassName("chaos-link-19")[key].getAttribute('chaos-id-market').length > 0) { // 判断是否填写了市场房间号
                                        roomID = document.getElementsByClassName("chaos-link-19")[key].getAttribute('chaos-id-market');
                                    }
                                }
                                document.getElementsByClassName("chaos-link-19")[key].href = 'http://19.offcn.com/class-' + roomID + '/?area=' + ChaosLoacation.Key419;
                                count++;
                            }
                        });
                    ...
                    // 填充 '专题' 定位后缀
                    if (typeof ChaosLoacation.Key === 'string') {
                        ...
                        Object.keys(document.getElementsByClassName("chaos-link-article")).forEach(function (key) {
                            document.getElementsByClassName("chaos-link-article")[key].href = document.getElementsByClassName("chaos-link-article")[key].href + ChaosLoacation.Key;
                            count++;
                        });
                    ...
                    // 填充定位名称
                    if (typeof ChaosLoacation.Key === 'string') {
                        ChaosFunctions.Logger({ Type: 'info', Info: '开始填充 「 定位名称 」' });
                        let count = 0;
                        Object.keys(document.getElementsByClassName("chaos-postfix-name")).forEach(function (key) {
                            document.getElementsByClassName("chaos-postfix-name")[key].value = ChaosLoacation.Name;
                            // document.getElementsByClassName("chaos-postfix-name")[key].setAttribute("value", ChaosLoacation.Name);
                            count++;
                        });
                        ChaosFunctions.Logger({ Type: 'info', Info: '「 定位名称 」填充完成，共填充 ' + count + ' 个。' });
                    }
                    ...
                    initTimer = setInterval(function () {
                        if (typeof (Cookies) === "function" && typeof ChaosXHR === "object") {
                            ...
                            if ($_GET['ticket'] !== undefined) {
                                ChaosXHR.GET(ChaosApiPath + "/one-time-pad/status/" + $_GET['ticket'], function (xhr) {
                                    if (typeof (xhr.responseJson) == "object") {
