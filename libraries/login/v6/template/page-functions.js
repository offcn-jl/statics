(function () {
    ...
    // 填充定位
    Object.keys(document.getElementsByClassName("offcn-sso-ascription")).forEach(function (下标) {
        document.getElementsByClassName("offcn-sso-ascription")[下标].innerHTML = 定位;
    });
                ...
                哈士齐单点登陆模块.公共函数.XHR.POST(哈士齐单点登陆模块.公共信息.接口.GAEA + "/events/sso/sign-up", Object.assign({ MID: 哈士齐单点登陆模块.公共信息.模块ID, Suffix: 哈士齐单点登陆模块.公共信息.宣传后缀, Phone: 用户手机号, Code: 验证码 * 1 /* 修正为数字类型 */, URL: window.location.href }, 获取可选参数的值())/* 将基本的参数对象与可选参数对象进行合并 */, function (响应内容) {
                    if (typeof (响应内容.responseJson) == "object") {
                        if (响应内容.responseJson.Message !== "Success") {
                            ...
                            哈士齐单点登陆模块.公共函数.隐藏元素("offcn-sso-login-page-form");
    ...
    哈士齐单点登陆模块.公共函数.打印日志({ 类型: '信息', 内容: "页面嵌入式登陆中间件处理函数 ( page-functions.js ) 加载成功" });
    ...
    function 切换登陆状态(用户需要注册) {
        var 提示内容 = '<span class="offcn-sso-checked"></span>您的手机号已经注册, 直接点击提交即可完成操作!';
        if (用户需要注册) {
            提示内容 = '<span class="offcn-sso-uncheck"></span> 您的手机号未注册, 请获取并填写验证码!';
    ...
    function 获取可选参数的值() {
        var 可选参数的值 = {}
        if (document.getElementById("offcn-sso-customer-name") !== null && document.getElementById("offcn-sso-customer-name").value !== "") {
            可选参数的值.CustomerName = document.getElementById("offcn-sso-customer-name").value;
        }
        if (document.getElementById("offcn-sso-customer-identity") !== null && document.getElementById("offcn-sso-customer-identity").value * 1 !== 0 && !isNaN(document.getElementById("offcn-sso-customer-identity").value * 1)) {
            可选参数的值.CustomerIdentityID = document.getElementById("offcn-sso-customer-identity").value * 1;
        }
        if (document.getElementById("offcn-sso-customer-colleage") !== null && document.getElementById("offcn-sso-customer-colleage").value !== "") {
            可选参数的值.CustomerColleage = document.getElementById("offcn-sso-customer-colleage").value;
        }
        if (document.getElementById("offcn-sso-customer-mayor") !== null && document.getElementById("offcn-sso-customer-mayor").value !== "") {
            可选参数的值.CustomerMayor = document.getElementById("offcn-sso-customer-mayor").value;
        }
        if (document.getElementById("offcn-sso-remark") !== null && document.getElementById("offcn-sso-remark").value !== "") {
            可选参数的值.Remark = document.getElementById("offcn-sso-remark").value;
        }
        return 可选参数的值
    }
})();
