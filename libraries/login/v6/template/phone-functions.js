    ...
    document.getElementById("offcn-sso-submit").addEventListener("click", function () {
        ...
        if (验证手机号(用户手机号)) {
            ...
            if (哈士齐单点登陆模块.公共信息.用户需要注册) {
                ...
                // 都合法则继续提交
                哈士齐单点登陆模块.公共函数.XHR.POST(哈士齐单点登陆模块.公共信息.接口.GAEA + "/events/sso/sign-up", { MID: 哈士齐单点登陆模块.公共信息.模块ID, Suffix: 哈士齐单点登陆模块.公共信息.宣传后缀, Phone: 用户手机号, Code: 验证码 * 1 /* 修正为数字类型 */ }, function (响应内容) {
                    if (typeof (响应内容.responseJson) == "object") {
                        if (响应内容.responseJson.Message !== "Success") {
                            alert(响应内容.responseJson.Message)
                        } else {
                ...
                // 调用登陆接口
                哈士齐单点登陆模块.公共函数.XHR.POST(哈士齐单点登陆模块.公共信息.接口.GAEA + "/events/sso/sign-in", { MID: 哈士齐单点登陆模块.公共信息.模块ID, Suffix: 哈士齐单点登陆模块.公共信息.宣传后缀, Phone: 用户手机号 }, function (响应内容) {
                    if (typeof (响应内容.responseJson) == "object") {
                        if (响应内容.responseJson.Message !== "Success") {
                            alert(响应内容.responseJson.Message)
                        } else {
